...
int main(int argc, char *argv[]) {
    ...
    int i; // Для цикла приведения к нижнему регистру // // Объявляет целочисленную переменную i.  Используется как счетчик в цикле при преобразовании строки к нижнему регистру
    ...
    for (;;) { //// Начало бесконечного цикла for (который будет выполняться до тех пор, пока не будет прерван оператором break
