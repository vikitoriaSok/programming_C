...
int main(int argc, char *argv[]) {
    char answer[10]; // Объявляет массив символов (строку) с именем answer, способный хранить до 9 символов + символ конца строки '\0'.  
	//Используется для хранения ответа пользователя ("yes" или "no").
