...
/*
1. malloc(). Выделяет память длиной для определённого количества байт и возвращает указатель на начало выделенной памяти. 
    В случае неудачного выполнения возвращает NULL. 
2. calloc(). Выделяет память для определённого количества элементов заданного размера и инициализирует выделенную память так, чтобы все байты были равны нулю.
3. realloc(). Позволяет повторно использовать или расширять память, ранее выделенную с помощью malloc() или calloc().
